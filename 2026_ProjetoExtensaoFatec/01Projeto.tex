...
A identificação dos atores permite reconhecer quem interage com o sistema e quais objetivos cada participante busca alcançar por meio das funcionalidades oferecidas. Na modelagem de casos de uso, um ator representa um papel desempenhado por uma pessoa ou por outro elemento externo durante sua interação com a solução, não correspondendo necessariamente a um indivíduo específico \parencite{Guedes2018}. No contexto do \textit{Civitas}, foram identificados três perfis de acesso: Administrador, Funcionário e Visitante. Esses perfis compartilham comportamentos comuns de um usuário autenticado, mas possuem responsabilidades e níveis de atuação distintos.

O ator abstrato \textit{Usuário}, representado na Figura~\ref{fig:atores}, reúne as interações comuns aos três perfis. Entre elas estão a autenticação mediante e-mail e senha, o acesso às funcionalidades autorizadas, a consulta dos próprios dados de perfil e o encerramento da sessão. Esse ator não representa um quarto perfil de acesso; sua utilização no diagrama evita a repetição de comportamentos compartilhados e demonstra a relação de generalização com Administrador, Funcionário e Visitante.
...
O \textit{Visitante} corresponde ao usuário de acesso restrito, cuja participação é predominantemente consultiva. Esse perfil pode autenticar-se, visualizar o painel e consultar informações administrativas ou financeiras para as quais tenha autorização, além de pesquisar registros e acessar seus próprios dados de perfil. Por não exercer funções operacionais ou administrativas, o Visitante não possui permissão para cadastrar, atualizar, inativar ou excluir informações. A existência desse perfil permite disponibilizar dados necessários ao acompanhamento das atividades sem conceder privilégios de alteração.

A Figura~\ref{fig:atores} apresenta a relação de generalização entre os atores. As setas partem dos perfis específicos e apontam para o ator abstrato Usuário, indicando que Administrador, Funcionário e Visitante compartilham as interações básicas de acesso, mas conservam permissões compatíveis com suas respectivas responsabilidades. O próprio \textit{Civitas} não é representado como ator, pois constitui o sistema que está sendo modelado, e não um elemento externo que interage com ele.
